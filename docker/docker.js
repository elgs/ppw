...
export class Docker {
   ...
   init() {
      ...
      const dom = this.dom;
      dom.style['z-index'] = Number.MAX_SAFE_INTEGER - 1000;
      ...
      this.y = 30;
      this.z = 0;
      this.sortable = az.ui(Sortable, this.dom, {
         // placeholder: true,
         create: (e, target) => {
   ...
   getContextMenuItems(dockId, winSettings) {
      ...
      return function () {
         // console.log(dockId);
         // console.log(me.dom);
         const docked = me.dom.querySelector(`[az-dock-id='${dockId}']:not(.az-placeholder)`);
         // console.log(docked);
   ...
   dock(el, winSettings, notify) {
      ...
      const cm = az.ui(ContextMenu, docked, {
      ...
      const clicked = e => {
         ...
         const docked = me.dom.querySelector(`[az-dock-id='${id}']:not(.az-placeholder)`);
   ...
   activate(dockId, notify) {
      // docker, window two way notify
      const me = this;
      this.dom.querySelectorAll('.azSortableItem').forEach(el => {
      ...
      const docked = this.dom.querySelector(`[az-dock-id='${dockId}']:not(.az-placeholder)`);
